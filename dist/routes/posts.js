...
import { buildSignedPlaybackToken, getCloudflareVideo, resolvePlaybackUrls } from "../lib/cloudflare.js";
import { countWords, toApiPost, toApiPostWithVideoPlayback } from "../lib/posts.js";
...
async function fetchPostPage(params) {
    ...
    const rows = unwrapData(result, "Failed to load posts");
...
export async function registerPostRoutes(fastify) {
    fastify.register(async (app) => {
        ...
        app.post("/posts", { preHandler: requireAuth }, async (request, reply) => {
            ...
            if (parsed.data.media.type === "video") {
                ...
                const ownershipResult = await supabaseAdmin
                    ...
                    .maybeSingle();
                const ownedUpload = unwrapOptionalData(ownershipResult);
            ...
            const insertResult = await supabaseAdmin
                ...
                .select("id,author_id,title,description,topic,location,hashtags,media_type,media_url,thumbnail_url,cloudflare_uid,like_count,bookmark_count,view_count,comment_count,share_count,created_at,updated_at")
                .single();
            const createdPost = unwrapData(insertResult, "Failed to create post");
            const videoPlayback = createdPost.media_type === "video" && createdPost.cloudflare_uid
                ? await (async () => {
                    const uid = createdPost.cloudflare_uid;
                    const signedToken = await buildSignedPlaybackToken(uid);
        ...
        app.post("/posts/:postId/likes", { preHandler: requireAuth }, async (request, reply) => {
            ...
            unwrapData(toggleResult, "Failed to toggle like");
            return { ok: true };
        });
        app.post("/posts/:postId/saves", { preHandler: requireAuth }, async (request, reply) => {
            ...
            unwrapData(toggleResult, "Failed to toggle save");
            return { ok: true };
        });
        app.post("/posts/:postId/share", { preHandler: requireAuth }, async (request, reply) => {
            ...
            const shareCount = unwrapData(incrementResult, "Failed to increment share count");
        ...
        app.get("/posts/:postId/comments", async (request, reply) => {
            ...
            const rows = unwrapData(result, "Failed to load comments");
        ...
        app.post("/posts/:postId/comments", { preHandler: requireAuth }, async (request, reply) => {
            ...
            const rows = unwrapData(createResult, "Failed to create comment");
        ...
        app.get("/me/likes", async (request, reply) => {
            ...
            const rows = unwrapData(result, "Failed to fetch likes");
            return {
        ...
        app.get("/me/saves", async (request, reply) => {
            ...
            const result = await supabaseAdmin
                ...
                .select("post_id")
                .eq("user_id", authUser.id);
            const rows = unwrapData(result, "Failed to fetch saves");
            return {
                postIds: rows.map((row) => row.post_id)
            };
        });
