...
async function loadVideoUpload(uid: string) {
  ...
  return unwrapOptionalData<VideoUploadRow>(
    result as {
      data: VideoUploadRow | null;
      error: { code?: string; message: string } | null;
    }
  );
}

function verifyUploadOwnership(existingUpload: VideoUploadRow | null, requestUserId: string) {
...
export async function registerMediaRoutes(fastify: FastifyInstance) {
  fastify.register(async (app) => {
    const handleVideoRead = async (request: FastifyRequest, reply: FastifyReply) => {
